...
/**
 * Verifies the "none" attestation statement format per WebAuthn L3.
 *
 * For `fmt: "none"`, the attestation statement (`attStmt`) must be an empty CBOR map.
 * Other formats are rejected with an "unsupported" error until dedicated verifiers are added.
 */
...
internal fun parseAttestationObject(bytes: ByteArray): ParsedAttestationObject? {
    ...
    return if (fmt != null && attStmtEntryCount != null) {
        ParsedAttestationObject(fmt = fmt!!, attStmtEntryCount = attStmtEntryCount!!)
    } else {
        null
    }
}

// ---- Minimal CBOR helpers (same logic as serialization module, kept self-contained) ----
