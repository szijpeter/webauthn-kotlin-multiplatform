...
- Finish payloads contain only the credential response. The routes map it to a byte-preserving raw response, derive `type`, `challenge`, and `origin` from its signed `clientDataJSON`, then call server-core; do not send duplicate client-data fields.
