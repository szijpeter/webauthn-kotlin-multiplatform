...
class AndroidRestoreCredentialClientTest {
    ...
    @Test
    fun createRestoreCredential_maps_dom_exception_to_invalid_options() = runBlocking {
        val credentialManager = mockk<CredentialManager>(relaxed = true)
        coEvery {
            credentialManager.createCredential(any<Context>(), any<CreateRestoreCredentialRequest>())
        } throws CreateRestoreCredentialDomException(DataError(), "invalid restore options")
        val client = testClient(credentialManager)
        ...
        assertTrue(failure.error.message.contains("invalid restore options"))
    }
