...
class IosPasskeyClientImplTest {
    ...
    @Test
    fun createCredential_maps_mismatched_credential_ids_to_Platform() = runBlocking {
        val bridge = FakeAuthorizationBridge(
            createResult = Result.success(
                IosRegistrationPayload(
                    credentialId = decode("AQID"),
        ...
        val result = IosPasskeyClientImpl(bridge).createCredential(mockOptions())

        assertTrue(result is PasskeyResult.Failure)
        assertTrue(result.error is PasskeyClientError.Platform)
        assertTrue(result.error.message.contains("credentialId must match rawId"))
    }

    @Test
    fun createCredential_maps_unknown_authenticator_attachment_to_Platform() = runBlocking {
        val bridge = FakeAuthorizationBridge(
            createResult = Result.success(
                IosRegistrationPayload(
                    credentialId = decode("AQID"),
        ...
        assertTrue(result is PasskeyResult.Failure)
        assertTrue(result.error is PasskeyClientError.Platform)
        assertTrue(result.error.message.contains("Unsupported authenticator attachment"))
    }
