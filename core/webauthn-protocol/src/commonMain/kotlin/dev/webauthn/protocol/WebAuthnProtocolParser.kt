...
public data class ParsedAuthenticatorData(
    ...
    public val extensionDataBytes: Base64UrlBytes?,
)
...
public object WebAuthnProtocolParser {
    ...
    public fun parseRegistrationResponse(value: RawRegistrationResponse): ValidationResult<RegistrationResponse> {
        ...
        val parsedAuthData = when (
            val result = parseAuthenticatorData(authDataBytes.bytes(), "attestationObject.authData")
        ) {
            is ValidationResult.Valid -> result.value
            is ValidationResult.Invalid -> return result
        }
    ...
    public fun extractAuthenticatorData(
        ...
        return authData?.let { ValidationResult.Valid(Base64UrlBytes.fromBytes(it)) }
...
private fun parseExtensionData(bytes: ByteArray, offset: Int, field: String): ValidationResult<Base64UrlBytes> {
    ...
    return ValidationResult.Valid(Base64UrlBytes.fromBytes(bytes.copyOfRange(offset, extensionEnd)))
}
