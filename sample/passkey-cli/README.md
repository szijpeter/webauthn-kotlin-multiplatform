...
This creates `sample/passkey-cli/.venv` and installs dependencies from `requirements.txt`.
When this venv exists, the CLI auto-selects `sample/passkey-cli/.venv/bin/python` unless `--python-bin` is explicitly provided.
...
- `--python-bridge <path>` (`ctap` mode only; default resolves to `sample/passkey-cli/scripts/fido2_bridge.py`)
