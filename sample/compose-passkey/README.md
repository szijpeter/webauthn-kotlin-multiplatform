...
The signed-in demo screen includes a Restore Credentials card. On Android it uses the published
`webauthn-client-platform` restore client directly, so the Android source-set API is exercised by a
visible flow rather than only by host tests. On iOS the same shared card is visible but disabled and
explains that the feature is unavailable.
...
The Android feature requires Android 9+, Google Play services core 24220000+, and
`androidx.credentials` 1.5+. It supports one restore account per application. Although this demo
reuses the passkey verification endpoints, production servers should label restore keys separately
from user-managed passkeys and apply restore-specific lifecycle policy.
