...
The inventory currently defers application-neutral ceremony orchestration and any optional native transport.
Orchestration is the next public capability: it must preserve opaque backend state, ceremony phase order,
single-operation behavior, cancellation, and backend verification ordering without taking ownership of UI or
persistence. Native transport remains behind a consumer-evidence gate and must not mirror Ktor concepts merely
to make module names match. Deferred entries expire on their recorded review date so these decisions cannot
...
client-surface parity checks, Swift package validation, Debug tests, a Release library-evolution build, and the
