...
/**
 * Temporary backend contract for the sample app (`temp.server`) route shape.
 * Remove once sample backend aligns with the default library contract.
 */
internal class TempServerBackendContract(
    private val registerOptionsPath: String = "/register/options",
    private val registerVerifyPath: String = "/register/verify",
    private val authenticateOptionsPath: String = "/authenticate/options",
    private val authenticateVerifyPath: String = "/authenticate/verify",
) : BackendContract {
    private val registrationChallengeToContext: MutableMap<String, PendingRegistrationContext> = mutableMapOf()
    ...
    override suspend fun getRegisterOptions(
        ...
        registrationChallengeToContext[response.challenge] = PendingRegistrationContext(
            userId = params.userHandle,
            userName = params.userName,
        )
    ...
    override suspend fun finishRegister(
        ...
        return runCatching {
            val httpResponse = httpClient.post("${endpointFor(registerVerifyPath)}$query") {
                header(HttpHeaders.ContentType, ContentType.Application.Json.toString())
                setBody(payload)
            }
    ...
    override suspend fun finishSignIn(
        ...
        return runCatching {
            val httpResponse = httpClient.post(
                "${endpointFor(authenticateVerifyPath)}?challenge=$challengeAsBase64Url",
            ) {
                header(HttpHeaders.ContentType, ContentType.Application.Json.toString())
                setBody(payload)
            }
...
private data class PendingRegistrationContext(
    val userId: String,
    val userName: String,
)
