...
```bash
./gradlew :app:compose-passkey:allTests \
  :app:compose-passkey:compileAndroidMain \
  :app:compose-passkey:compileKotlinIosSimulatorArm64 \
  :app:compose-passkey-android:compileDebugAndroidTestKotlin \
  :app:compose-passkey-android:lintDebug \
  :app:compose-passkey-android:assembleDebug --stacktrace
```
...
- Android UI smoke test sources compile in CI (`:app:compose-passkey-android:compileDebugAndroidTestKotlin`).
...
```bash
./gradlew :app:compose-passkey-android:installDebug
...
```bash
./gradlew :app:compose-passkey-android:connectedDebugAndroidTest --stacktrace
```
