...
Compose Multiplatform sample app for a minimal passkey E2E flow against `app/backend-ktor`.
...
```bash
WEBAUTHN_DEMO_ENDPOINT=http://10.0.2.2:8080 ./gradlew :app:compose-passkey-android:installDebug
```

3. Optional UI smoke test (emulator/device connected):

```bash
./gradlew :app:compose-passkey-android:connectedDebugAndroidTest
```
...
- [`app/compose-passkey-ios`](../compose-passkey-ios/README.md)

Quick start:

1. Open `app/compose-passkey-ios/ComposePasskeyIos.xcodeproj` in Xcode.
...
- `app/backend-ktor/start-server.sh` (ngrok helper) remains the default physical-device setup path.
...
- `app:compose-passkey` `commonTest` validates flow behavior with `FakePasskeyClient` and `FakeServerClient` (`PasskeyServerClient`) so orchestration is deterministic across KMP targets.
...
`app/backend-ktor` serves both endpoints.
