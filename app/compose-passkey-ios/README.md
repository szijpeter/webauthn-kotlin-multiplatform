...
Ready-to-run iOS host app for the shared `app:compose-passkey` Compose UI.
...
- SwiftUI app shell that mounts Kotlin `MainViewController()` from `app:compose-passkey`.
- Build phase script that runs `:app:compose-passkey:embedAndSignAppleFrameworkForXcode`.
...
```bash
open app/compose-passkey-ios/ComposePasskeyIos.xcodeproj
```
...
- Build phase cannot find Gradle task:
  - Run from repo root and ensure `app:compose-passkey` iOS framework targets are configured.
...
```bash
xcodegen generate --spec app/compose-passkey-ios/project.yml
```
