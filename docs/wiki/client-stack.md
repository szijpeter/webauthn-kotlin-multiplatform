...
The client side is organized around a raw-response boundary in common Kotlin code, with thin platform bridges on Android and iOS.

## Core Shape

- `webauthn-client-core` owns platform option checks, cancellation/error mapping, and raw client APIs.
...
2. Shared client orchestration maps those inputs into a platform-ready flow.
...
- JSON interop is optional and separate from the raw core.
