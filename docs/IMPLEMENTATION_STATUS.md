...
- Network interop uses default `/webauthn/*` route wiring in `webauthn-network-ktor-client`, path overrides via `KtorPasskeyRoutes`, and first-party sample backend routes under `sample/backend-ktor`.
...
| `webauthn-server-ktor` | Beta | Thin route adapters + tests; finish routes map transport DTOs to single-authority raw requests and accept no duplicate client-data payload fields, while server-core derives signed ceremony data. | Operational hardening and sample-level integration depth |
...
| `webauthn-client-android` | Beta | Thin Credential Manager bridge, replaceable JSON codec injection, byte-preserving raw responses, deterministic platform error mapping, targeted RP ID validation troubleshooting hints (invalid-options and platform paths), capability reporting, shared-core delegation | OEM/provider compatibility matrix hardening |
| `webauthn-client-ios` | Beta | Thin AuthenticationServices bridge, byte-preserving raw responses without a JSON implementation dependency, deterministic NSError mapping, capability reporting, shared-core delegation, PRF extension input/output bridge wiring, platform-first attachment selection when caller leaves attachment unset | More runtime/device matrix coverage |
