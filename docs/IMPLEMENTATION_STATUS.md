...
| `webauthn-server-jvm-crypto` | Beta | JCA/JCE crypto baseline + `none`/`packed` (incl. AAGUID) + `android-key`/`tpm`/`apple` + `android-safetynet` scaffold + dispatcher integration | Additional attestation formats (none missing?), trust-chain depth |
