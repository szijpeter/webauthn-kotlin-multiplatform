...
| `webauthn-client-android` | Beta | Thin Credential Manager bridge, replaceable JSON codec injection, raw-response protocol interpretation, deterministic platform error mapping, targeted RP ID validation troubleshooting hints (invalid-options and platform paths), capability reporting, shared-core delegation | OEM/provider compatibility matrix hardening |
| `webauthn-client-ios` | Beta | Thin AuthenticationServices bridge, raw-response protocol interpretation without a JSON implementation dependency, deterministic NSError mapping, capability reporting, shared-core delegation, PRF extension input/output bridge wiring, platform-first attachment selection when caller leaves attachment unset | More runtime/device matrix coverage |
