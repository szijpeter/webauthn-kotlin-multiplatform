...
| `webauthn-protocol` | Beta | Codec-neutral interpretation of raw authenticator data and structural `authData` extraction from attestation objects, with immutable `Base64UrlBytes` outputs and only typed-model/strict-CBOR dependencies | Wider parser/vector coverage and higher-level raw-to-clean response conversion |
