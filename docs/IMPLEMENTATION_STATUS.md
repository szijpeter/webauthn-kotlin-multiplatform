...
- Documentation verification hardening (2026-08-01): authentication examples now require an explicit
  signature-verification result before returning `Valid`; directive parsing rejects malformed or ambiguous
  metadata with source locations; owner/verification combinations are compatibility-checked; and CI checkout
  credentials are not persisted. These changes improve documentation and build security without changing the
  library's runtime or public API contracts.
- Documentation verification revalidation (2026-08-01): the branch was refreshed onto the current AGP 9.3.1
  and Ktor 3.5.2 baseline before the follow-up validation pass.
- Kotlin 2.4 feature-adoption pass (2026-06-25): enabled preview collection literals in shared/JVM build logic and adopted them across core, model, serialization, client, server, and sample modules where they improved clarity without changing behavior. Name-based destructuring was re-reviewed but intentionally not enabled because the low-risk `only-syntax` mode is more verbose than the existing code, while the concise `complete` mode would widen preview semantics across 26 existing destructuring sites for only slight readability gain in this public library. Compiler-proven collection-literal carveouts remain in `webauthn-client-platform` and `sample:compose-passkey`, where the active toolchain still rejects specific rewrites.
...
- P6-003 implementation update (2026-04-17): authentication start is now unified with optional `userName` across server core + Ktor route + Ktor transport, challenge/session stores support nullable authentication `userName`, and named vs discoverable finish behavior is explicitly validated (credential ownership enforced for named mode, credential-id resolution for discoverable mode). Auth-start transport payloads dropped `userHandle`; sample sign-in now defaults to discoverable start while sample registration requests `residentKey=required`.
...
- Compose integration helpers now provide `rememberPasskeyClient` plus state-free `rememberPasskeyFlow`; the legacy controller helper is deprecated while transport/sample migration completes.
...
- Network interop uses default `/webauthn/*` route wiring in `webauthn-client-ktor`, path overrides via `KtorPasskeyRoutes`, and first-party sample backend routes under `sample/backend-ktor`.
...
- Client capability reporting refactor (2026-08-15): `PasskeyCapability` now uses typed `Extension` and `Platform` variants, `PlatformCapability` provides typed standard and custom platform features, and `PasskeyCapabilities` reports `SUPPORTED`, `UNSUPPORTED`, or `UNKNOWN` without leaking string version hints.
...
|---|---|---|---|
| `webauthn-model` | Production-leaning | Typed protocol models, strict base64url behavior, value semantics tests, immutable byte/domain wrappers for binary protocol values, redacted byte-wrapper diagnostics, named `ClientDataHash`/`CosePublicKey` values, raw response contracts that preserve untrusted platform/transport output before protocol interpretation, L3 extension models (PRF eval/evalByCredential, LargeBlob read/write, Related Origins), authenticator transports/attachment and attestation preference models | Continued edge-case coverage for uncommon protocol combinations |
...
| `webauthn-json-api` | Beta | Replaceable WebAuthn-specific JSON codec boundary for typed options, raw credential responses, and signed client-data decoding | Adoption by transport and client facades without selecting an implementation |
| `webauthn-protocol` | Beta | Codec-neutral interpretation of raw authenticator data, structural `authData` extraction from attestation objects, and raw-to-clean registration/assertion response conversion using only typed models and the strict CBOR scanner | Wider parser/vector coverage |
...
| `webauthn-json-kotlinx` | Beta | DTO mapping and signed `clientDataJSON` parsing for transport adapters, with round-trip tests, attachment/attestation/transports mapping, canonical response JSON emission (`type` + `clientExtensionResults`) and a null-tolerant `allowCredentials` decode shim. Binary protocol interpretation belongs to `webauthn-protocol`. | Wider cross-implementation differential coverage beyond current JVM fixture set |
...
| `webauthn-server-ktor` | Beta | Thin route adapters + tests; finish routes map transport DTOs to raw responses, derive ceremony type, challenge, and origin from signed `clientDataJSON`, and accept no duplicate client-data payload fields. | Operational hardening and sample-level integration depth |
| `webauthn-client-core` | Beta | Raw platform-client boundary (`PasskeyClient` and `DefaultPasskeyClient`) with byte-preserving responses, deterministic invalid-options vs platform error behavior, coroutine cancellation passthrough, capability model, and no protocol-parser dependency | More extension-focused policy helpers and fixture coverage |
| `webauthn-client-defaults` | Beta | Recommended Android/iOS platform client composition with the Kotlinx JSON codec, while preserving injectable Android codec configuration and an opt-in Ktor transport | Default consumer smoke coverage and migration of existing samples |
| `webauthn-client-json-core` | Beta | Optional raw JSON client APIs (`JsonPasskeyClient`) using the replaceable `WebAuthnJsonCodec` contract, with the Kotlinx codec as a default and JVM-side Yubico JSON interop tests for options/response payload compatibility | Additional fixture depth and profile-oriented JSON interop coverage |
| `webauthn-client-compose` | Beta | Compose integration helpers (`rememberPasskeyClient`, `rememberPasskeyFlow`) for caller-owned UI state and retained-VM-safe prompt resolution; deprecated controller compatibility remains during migration | Broader UI/runtime lifecycle coverage across host app patterns |
...
| `webauthn-client-ktor` | Production-leaning | Transport helper client + payload tests, Related Origins fetcher, default `/webauthn/*` route wiring plus `KtorPasskeyRoutes` path overrides, optional extension transport fields on start payloads | Retry/error policy hardening and broader contract fixtures |
...
- Broader discoverable credential conformance matrix (additional fixture/differential vectors across adapters and stores).
