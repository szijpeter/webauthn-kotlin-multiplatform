...
| `webauthn-server-jvm-crypto` | Beta | JCA/JCE crypto baseline + `none`/`packed`/`android-key` trust-path (Google/Apple roots configured) + `tpm`/`apple` + `android-safetynet` scaffold + dispatcher integration | Additional attestation formats (none missing?), `android-key` depth |
