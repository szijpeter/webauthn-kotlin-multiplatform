...
| `webauthn-server-ktor` | Beta | Thin route adapters + tests; finish routes derive ceremony type, challenge, and origin from signed `clientDataJSON` and accept no duplicate client-data payload fields. | Operational hardening and sample-level integration depth |
