...
- Exhaustive collection-literal follow-up (2026-06-25): completed a branch-wide second pass over remaining `listOf`/`setOf` candidates in touched Kotlin sources and converted every compiler-safe site we could confirm. Nine call sites intentionally remain on `listOf`/`setOf`: two Android bridge calls in `webauthn-client-android` and seven `sample:compose-passkey` commonMain/commonTest sites, all of which currently fail compilation or trigger a Kotlin compiler analysis error when rewritten as collection literals under the module's active toolchain.
