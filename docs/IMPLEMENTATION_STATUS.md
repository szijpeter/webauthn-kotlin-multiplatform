...
| `webauthn-server-jvm-crypto` | Beta | JCA/JCE crypto baseline + `none` and `packed` attestation format verification (self + full/x5c) + shared CBOR parser + tests + `android-key` validation (software/teeEnforced) | Additional attestation formats (`tpm`, `apple`, `android-safetynet`), AAGUID extension validation, trust-chain depth |
