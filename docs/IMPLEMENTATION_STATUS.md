...
| `webauthn-server-jvm-crypto` | Beta | JCA/JCE crypto baseline + `none` and `packed` attestation format verification + shared CBOR parser + tests + `android-key` validation + `tpm`/`android-safetynet` scaffold + `apple` | Additional attestation formats (none missing?), AAGUID extension validation, trust-chain depth |
