...
| `webauthn-server-jvm-crypto` | Beta | JCA/JCE crypto baseline + `none` and `packed` attestation format verification + shared CBOR parser + tests + `android-key` validation + `tpm`/`apple` | Additional attestation formats (`android-safetynet`), AAGUID extension validation, trust-chain depth |
