...
5. Expose `capabilities()` with typed `PasskeyCapabilities.supportOf(...)` values so callers can distinguish supported, unsupported, and unknown runtime capability status (PRF, Large Blob, security key).
